...
tstring CIni::GetString(LPCTSTR lpSection, LPCTSTR lpKey, tstring & defaultStr) const
{
	LPTSTR psz = __GetStringDynamic(lpSection, lpKey, defaultStr.c_str());
    return tstring(psz);
}

tstring CIni::GetString(LPCTSTR lpSection, LPCTSTR lpKey) const
{
    return GetString(lpSection, lpKey, tstring());
}
...
bool CIni::WriteString(LPCTSTR lpSection, LPCTSTR lpKey, LPCTSTR lpValue) const
{
	...
	return ::WritePrivateProfileString(lpSection, lpKey, lpValue == NULL ? _T("") : lpValue, m_pszPathName) != 0;
}
...
{
	return ::WritePrivateProfileString(lpSection, NULL, _T(""), m_pszPathName) != 0;
}

// Remove a key from a section
bool CIni::DeleteKey(LPCTSTR lpSection, LPCTSTR lpKey) const
{
	return ::WritePrivateProfileString(lpSection, lpKey, NULL, m_pszPathName) != 0;
}
...
bool CIni::CopySection(LPCTSTR lpSrcSection, LPCTSTR lpDestSection, bool bFailIfExist) const
{
	...
	const bool RES = ::WritePrivateProfileSection(lpDestSection, psz, m_pszPathName) != 0;
	delete [] psz;
...
{
	const size_t LEN = lpEnd - lpStart;
	LPTSTR psz = new TCHAR[LEN + 1];
